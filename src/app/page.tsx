...
export default function RadarPage() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      novas: allJobs.filter((j) => {
        const posted = j.postedAt ? new Date(j.postedAt).getTime() : 0;
        return posted > fortyEightHoursAgo && posted > 0;
      }).length,
  ...
  const highFitJobs = useMemo(() => {
    return filteredNewJobs.filter((j) => j.fitLabel === "high" && (j.score ?? 0) >= 0.75).slice(0, 10);
  }, [filteredNewJobs]);

  const moderateFitJobs = useMemo(() => {
    return filteredNewJobs.filter((j) => (j.score ?? 0) >= 0.60 && (j.score ?? 0) < 0.80).slice(0, 10);
  }, [filteredNewJobs]);

  const recentJobs = useMemo(() => {
    return filteredNewJobs.sort((a, b) => new Date(b.postedAt || "").getTime() - new Date(a.postedAt || "").getTime()).slice(0, 30);
  }, [filteredNewJobs]);
  ...
  const briefing = useMemo(() => {
    ...
    const followUpsOverdue = allJobs.filter((j) => {
      ...
      return (nowMs - new Date(j.updatedAt).getTime()) > (settings.followUpDays || 5) * 86400000;
    });
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6">
          {[
            ...
            <div key={s.label} className="rounded-xl border bg-bg p-4">
              ...
              <p className={`text-2xl font-semibold mt-1 tabular-nums ${s.color}`}>{isLoading ? "—" : s.value}</p>
            </div>
          ))}
        </div>
          ...
          <>
            ...
            {highFitJobs.length === 0 && hasFilters && (
              <div className="mb-8 rounded-xl border border-dashed p-8 text-center">
                <p className="text-sm text-text-secondary">Nenhuma vaga de alto fit com estes filtros</p>
            ...
            {moderateFitJobs.length > 0 && (
              <div className="mb-8">
                <h2 className="text-sm font-semibold text-text-primary mb-3 flex items-center gap-2">
                  ...
                  Fit Moderado (60-79%) — Vale considerar
                </h2>
