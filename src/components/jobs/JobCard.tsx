...
import {
  ...
  Send, Sparkles, AlertTriangle, Archive, ArrowUpCircle,
} from "lucide-react";
    ...
    <Link href={`/jobs/${job.id}`}>
      <Card className={cn(
        ...
        <CardContent className={cn(isCompact ? "pb-3 pt-3" : "pb-4 pt-4")}>
          ...
          {/* Quick actions */}
          {!isCompact && transitions.length > 0 && (
            <div className="flex items-center gap-1.5 mt-3 pt-2 border-t border-border opacity-0 group-hover:opacity-100 transition-opacity flex-wrap">
              ...
              {job.status !== "archived" && job.status !== "ignored" && (
                <Button variant="ghost" size="sm"
                  className="text-[11px] gap-1 h-7 text-text-tertiary ml-auto"
                  onClick={(e) => handleAction(e, "archived")}>
                  <Archive className="h-3 w-3" />Arquivar
                </Button>
              )}
              <div className="flex-1" />
              <Button variant="primary" size="sm" className="text-xs gap-1 h-7">
